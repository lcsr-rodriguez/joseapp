...
public class SupportFloatingActionsMenu extends com.getbase.floatingactionbutton.FloatingActionsMenu {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (getTranslationY() == 0) {
            ...
            int offset = (getMeasuredHeight() - fab.getMeasuredHeight() + shadowRadius-85) / 2;
    ...
    public static class Behavior extends CoordinatorLayout.Behavior<SupportFloatingActionsMenu> {
        ...
        private void updateFabTranslationForSnackbar(CoordinatorLayout parent, SupportFloatingActionsMenu fab, View snackbar) {
            ...
            if(translationY != this.mTranslationY) {
                ...
                if(Math.abs(translationY - this.mTranslationY) == (float)snackbar.getHeight()) {
                    ViewCompat.animate(fab).translationY(translationY).setInterpolator((Interpolator) AnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR).setListener((ViewPropertyAnimatorListener)null);
                } else {
        ...
        private void animateIn(SupportFloatingActionsMenu button) {
            ...
            if(Build.VERSION.SDK_INT >= 14) {
                ...
                ViewCompat.animate(button).alpha(1.0F).setInterpolator((Interpolator) AnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR).withLayer().setListener((ViewPropertyAnimatorListener)null).start();
            } else {
                Animation anim = android.view.animation.AnimationUtils.loadAnimation(button.getContext(), com.google.android.material.R.anim.design_snackbar_in);
                anim.setDuration(200L);
                anim.setInterpolator((Interpolator) AnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR);
        ...
        private void animateOut(final SupportFloatingActionsMenu button) {
            if(Build.VERSION.SDK_INT >= 14) {
                // removed the scale X & Y to avoid strange animation behavior with the FAB menu
                ViewCompat.animate(button).alpha(0.0F).setInterpolator((Interpolator) AnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR).withLayer().setListener(new ViewPropertyAnimatorListener() {
                ...
                Animation anim = android.view.animation.AnimationUtils.loadAnimation(button.getContext(), com.google.android.material.R.anim.design_snackbar_out);
                anim.setInterpolator((Interpolator) AnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR);
                anim.setDuration(200L);
